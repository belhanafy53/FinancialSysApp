...
{
    public partial class StoresFrm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            grbNodeSelected.Text = ViewTree.SelectFullHirachyStores(treeView1, Vint_IdItem, Vint_Parent_ID);
            //txtStoreCode.Text = listItem.StoreCode;
        }

        private void simpleButton1_Click(object sender, EventArgs e)
        {
            var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 56 && w.ProcdureId == 1);
            if (validationSaveUser != null)
            {
                if (txtSelected.Text == null)
                {
                    MessageBox.Show("برجاء ادخال المخزن المراد اضافته");
                }
                else
                {
                    ...
                    if (Vint_IdItem == 0)
                    {
                        ...
                        Tbl_Stores Itm = new Tbl_Stores
                        {
                            Name = txtSelected.Text,
                            StoreCode = txtStoreCode.Text
                        };
                        FsDb.Tbl_Stores.Add(Itm);
                        FsDb.SaveChanges();
            ...
            {
                MessageBox.Show("ليس لديك صلاحية اضافة  مخزن .. برجاء مراجعة مدير المنظومه");
            }
        }

        private void simpleButton2_Click(object sender, EventArgs e)
        {
            var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 56 && w.ProcdureId == 4);
            if (validationSaveUser != null)
            {
                var result1 = MessageBox.Show("هل تريد حدف هدا المخزن  ؟", "حدف مخزن " + " " + treeView1.SelectedNode.Text, MessageBoxButtons.YesNo);
                ...
                if (result1 == DialogResult.Yes)
                {
                    ...
                    var ListCheckDelete = FsDb.Tbl_Stores.FirstOrDefault(x => x.Parent_ID == Vint_IdItem);
                    if (ListCheckDelete == null)
                    {

                        var listDelete = FsDb.Tbl_Stores.FirstOrDefault(x => x.ID == Vint_IdItem);
                        if (listDelete != null)
                        {
                            FsDb.Tbl_Stores.Remove(listDelete);
                            FsDb.SaveChanges();
                            grbNodeSelected.Text = "";
                            txtSelected.Text = "";
                            //---------------خفظ ااحداث 
                    ...
                    {
                        MessageBox.Show("لايمكن حذف هدا المخزن لاحتوائه على بنود تابعه له قم بحدف تلك المخازن اولا");
                    }
        ...
        {
            var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 56 && w.ProcdureId == 3);
            if (validationSaveUser != null)
            {
                if (Vint_IdItem != null && txtSelected.Text != "")
                {
                    ...
                    var listupdate = FsDb.Tbl_Stores.FirstOrDefault(x => x.ID == Vint_IdItem);
            ...
            {
                //MessageBox.Show("ليس لديك صلاحية تعديل  مخزن .. برجاء مراجعة مدير المنظومه");
            }
        }
