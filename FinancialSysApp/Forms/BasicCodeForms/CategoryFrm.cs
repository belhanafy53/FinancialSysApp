...
namespace FinancialSysApp.Forms.BasicCodeForms
{
    public partial class CategoryFrm : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            dataGridView1.DataSource = FsDb.Tbl_RestrictionItemsCategory.ToList();
            dataGridView1.Columns["ID"].Visible = false;
            dataGridView1.Columns["Name"].HeaderText = "النوع";
            //dataGridView1.Columns["Tbl_Indebtedness"].Visible = false;
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dataGridView1.RowCount >= 1)
                {
                    ...
                    var result = FsDb.Tbl_RestrictionItemsCategory.SingleOrDefault(x => x.ID == xcatid);
        ...
        {
            dataGridView1.DataSource = FsDb.Tbl_RestrictionItemsCategory.Where(x => x.Name.Contains(Nametxt.Text)).ToList();
        }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 15 && w.ProcdureId == 1);
                    if (validationSaveUser != null)
                    {
                        Tbl_RestrictionItemsCategory CatF = new Tbl_RestrictionItemsCategory
                        {
                            Name = Nametxt.Text,

                        };
                        FsDb.Tbl_RestrictionItemsCategory.Add(CatF);
                        FsDb.SaveChanges();
                        //---------------خفظ ااحداث 
                        ...
                        SecurityEvent sev = new SecurityEvent
                        {
                            ...
                            TableName = "Tbl_RestrictionItemsCategory",
                ...
                {
                    var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 15 && w.ProcdureId == 3);
                    if (validationSaveUser != null)
                    {
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            try
            {
                var validationSaveUser = Program.SecurityProceduresList.FirstOrDefault(w => w.FormId == 15 && w.ProcdureId == 4);
                if (validationSaveUser != null)
                {
                    int xrows = dataGridView1.RowCount;
                    if (xrows != 0 && textBox1.Text != "")
                    {
                        ...
                        {
                            var result = FsDb.Tbl_RestrictionItemsCategory.Find(xcatid);
                            FsDb.Tbl_RestrictionItemsCategory.Remove(result);
                            FsDb.SaveChanges();
                            //---------------خفظ ااحداث 
